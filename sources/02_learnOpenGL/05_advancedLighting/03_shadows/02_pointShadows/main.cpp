...
int main()
{
    ...
    //----------------------------------------------------------------------------------
    // '0' 关闭PCF
    // '1' 开启PCF，使用当前片段周围的64个像素平均阴影
    // '2' 开启PCF，使用当前片段周围的20个像素平均阴影
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //------------------
        // 大立方体内部的一个小立方体，用来测试阴影
        modelMat = glm::translate(glm::mat4(1.f), glm::vec3(5.f, 5.f, 0.f)), glm::scale(glm::mat4(1.f), glm::vec3(.1f));
        depthProgram.SetUniformMat4("model", modelMat);
        cube.Draw(GL_TRIANGLES);

        modelMat = glm::translate(glm::mat4(1.f), glm::vec3(5.f, 5.f, 5.f)), glm::scale(glm::mat4(1.f), glm::vec3(.1f));
        depthProgram.SetUniformMat4("model", modelMat);
        cube.Draw(GL_TRIANGLES);
        ...
        lightProgram.SetUniform1i("shadowSoft", shadowSoft);
        lightProgram.SetUniform1i("diffuseTexture", 0);
        lightProgram.SetUniform1i("depthMap", 1);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_CUBE_MAP, depthTextureBuffer);
